		...
		if not osm_way['incomplete'] and osm_way['highway'] != None and osm_way['highway'] not in avoid_highway:
			...
			for nvdb_id, nvdb_way in iter(ways_nvdb.items()):
				...
				if nvdb_way['min_lat'] <= osm_way['max_lat'] and nvdb_way['max_lat'] >= osm_way['min_lat'] and \
					...
					if nvdb_way['highway'] in public_highway and osm_way['highway'] not in public_highway + ['tertiary'] or \
						osm_way['highway'] in public_highway and nvdb_way['highway'] not in public_highway + ['road', 'tertiary']:
						continue
...
#	start_time = time.time()
	...
	elif len(sys.argv) == 3 and ".osm" not in sys.argv[2].lower() and sys.argv[1].lower() in ["-new", "-offset", "-replace", "-tag"]:
		command = sys.argv[1].lower().strip("-")
		filename_osm = sys.argv[2]
